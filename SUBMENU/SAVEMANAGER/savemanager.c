...
/* Main function */
char *savemanager_show(const char **extensions) {
    char *result = NULL;
    ...
    while (1) {
        ...
        if (entry_count == 0 && mode == 1) {
            gfx_draw_text("Dir failed - replug USB?", 40, 200, GS_SETREG_RGBAQ(0xFF,0x00,0x00,0x80,0), 10, 4);
        }
        if (mode == 0) {
            ...
            if (pressed & PAD_CROSS && entry_count > 0) {
                ...
                if (e->is_dir) {
                    ...
                    if (manager_mode) {
                        /* TODO: Implement file operations (copy, paste, delete, rename, etc.) */
                        /* For now, do nothing or show placeholder */
                        gfx_draw_text("Manager ops not impl.", 40, 200, GS_SETREG_RGBAQ(0xFF,0xFF,0x00,0x80,0), 10, 4);
                        gfx_flip();
                        gfx_exec();
                        ...
                        if (ext_match(e->name, extensions)) {
                            ...
                            gfx_draw_text("Unsupported extension", 40, 200, GS_SETREG_RGBAQ(0xFF,0x00,0x00,0x80,0), 10, 4);
                            gfx_flip();
                            gfx_exec();
                            FuckAroundSilentlyMs(2000);
                        }

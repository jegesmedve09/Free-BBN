...
void update_and_draw_lava_background(void)
{
    frame_counter++;
...
void show_about(void)
{
	...
	{
		update_and_draw_lava_background();
		...
		gfx_draw_text("Press \xFF\x08 to go back", 5, 60+30+30+30+100+30+100, GS_SETREG_RGBAQ(0xFF,0xFF,0xFF,i,0x00), 5, 4);
		gfx_flip();
		gfx_exec();
	}
    ...
    while (1)
    {
		...
		gfx_draw_text("Press \xFF\x08 to go back", 5, 60+30+30+30+100+30+100, GS_SETREG_RGBAQ(0xFF,0xFF,0xFF,0x80,0x00), 5, 4);
		gfx_flip();
		gfx_exec();
...
int main(void)
{
	...
	for (int i = 0; i < 128; i+=2)
	{
		//gfx_flip();
        update_and_draw_lava_background();
        gfx_draw_line(0, 26, 640, 26, GS_SETREG_RGBAQ(0xFF, 0xFF, 0xFF, i, 0x00));
		gfx_draw_text("FreeBBN", 5, 5, GS_SETREG_RGBAQ(0xFF, 0xFF, 0xFF, i, 0x00), 5, 4);
		gfx_flip();
		gfx_exec();
	}
    ...
    for (int i = 0; i < 128; i+=2)
    {
		...
		gfx_draw_text("Main Menu", 40, 100, GS_SETREG_RGBAQ(0xFF, 0xFF, 0xFF, i, 0x00), 10, 4);	
		gfx_flip();
		gfx_exec();
	}
    ...
    for (int j = 0; j < 128; j+=2)
    {
		...
		for (int i = 0; i < MENU_ITEM_COUNT; i++)
		{
			...
			gfx_draw_text(menu_items[i], MENU_START_X, y, GS_SETREG_RGBAQ(0x90, 0x90, 0x90, j, 0x00), 8, 4);
		}
		gfx_flip();
		gfx_exec();
	}
    ...
    while(1)
    {
        ...
        for (int i = 0; i < MENU_ITEM_COUNT; i++)
		{
			...
			if (i == MENU_ITEM)
			{
				gfx_draw_text(menu_items[i], MENU_START_X, y, GS_SETREG_RGBAQ(0xFF, 0xFF, 0x00, 0x80, 0x00), 10, 4);
			}
			else
			{
				gfx_draw_text(menu_items[i], MENU_START_X, y, GS_SETREG_RGBAQ(0x90, 0x90, 0x90, 0x80, 0x00), 8, 4);
		...
		if(get_pad_buttons(0) & PAD_CROSS)
		{
			...
			{
				update_and_draw_lava_background();
				gfx_draw_line(0, 26, 640, 26, GS_SETREG_RGBAQ(0xFF, 0xFF, 0xFF, j, 0x00));
				gfx_draw_text("FreeBBN", 5, 5, GS_SETREG_RGBAQ(0xFF, 0xFF, 0xFF, j, 0x00), 5, 4);
				gfx_draw_text("Main Menu", 40, 100, GS_SETREG_RGBAQ(0xFF, 0xFF, 0xFF, j, 0x00), 10, 4);	
				for (int i = 0; i < MENU_ITEM_COUNT; i++)
				{
					int y = MENU_START_Y + i * MENU_LINE_HEIGHT;
					gfx_draw_text(menu_items[i], MENU_START_X, y, GS_SETREG_RGBAQ(0x90, 0x90, 0x90, j, 0x00), 8, 4);
